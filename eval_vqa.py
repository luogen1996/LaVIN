...
llm_model:str,
    ...
    set_Clip_Adapter(model.backbone, visual_adapter_type, dim=adapter_dim, s=adapter_scale,t=temperature)
...
def main(
    ...
    anns=json.load(open('../data/v2_OpenEnded_mscoco_val2014_questions.json'))['questions']
    #print(anns.keys())
    ...
    image_path = os.path.join(data_root, 'images', 'val2014')
    ...
    #     prefix="Below is an instruction that describes a task, paired with an input that provides further context.\n" \
    #        "Write a response that appropriately completes the request.\n\n"\
    #         "### Instruction:\n Using the provided context, question, and multiple options, please answer the question by selecting the correct option.\n\n###Input:\n"
    # else:
    prefix=''
    ...
    for i in range(total_items//max_batch_size+1):
        ...
        for qid in batch_qids:
            # prompt = build_prompt_pt(problems, qid, prompt_args)
            # prompt,_ = build_prompt(problems, qid, prompt_args)
            prompt='Instruction: '+anns[qid]['question']+'\n' + 'Response: '
            prompt=prompt.replace("  ", " ").strip()


            image = Image.open(os.path.join(image_path, 'COCO_val2014_%012d.jpg'%int(anns[qid]['image_id']))).convert('RGB')
            image = image_transforms(image)
            indicator = 1

            # predictions[qid]={}
            # predictions[qid]['gt'] = anns[qid]['caption']
            # predictions[qid]['image_path'] = os.path.join(image_path, anns[qid]['file_name'])
        ...
        results = generator.generate(
            prompts,images=images,indicators=indicators, max_gen_len=10, temperature=generation_temperature, top_p=top_p,n_feats=n_prompt
        )

        for j,result in enumerate(results):
            pred = result.split('Response:')[1]#pattern.findall(result)
            # print(j)
            # print(pred)
            # print(predictions[batch_qids[j]]['gt'])
            # print()
            # pred = pattern1.findall(result)+pattern2.findall(result)

            preds.append(pred)
            predictions.append({
                'question_id': anns[batch_qids[j]]['question_id'],
                'answer':pred
            })
            # predictions[batch_qids[j]]['pred']=pred



    with open('./vqa_preds.json', 'w') as f:
