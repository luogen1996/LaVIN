...
class ScienceQADataSet(Data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        answer, choices, qid = self.problems[self.qids[idx]]["answer"], self.problems[
            self.qids[idx]]["choices"], self.qids[idx]
    ...
    def __getitem__(self, idx):
        ...
        if self.data[idx]['image'] is not None:
            ...
            if self.data[idx]['image_source'] == 'sqa':
                ...
                image = Image.open(os.path.join('../data/images/train2014',
                                                'COCO_train2014_' + self.data[idx]['image'])).convert('RGB')
            image = self.transforms(image)
            indicator = 1
        else:
            image = torch.Tensor(torch.zeros(3, 224, 224).float())
