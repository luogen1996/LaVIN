...
def main(
    ...
    print('split: ', split)
    # problems = json.load(open('./qa90_questions.jsonl','r'))
    problems = [json.loads(q) for q in open(os.path.expanduser('./qa90_questions.jsonl'), "r")]
    problems = get_chunk(problems, 1, 0)
    # image_path = os.path.join(data_root, 'images','val2014')
    total_items=len(problems)
    print('total_items: ',len(problems))
    ...
    idxs = []
    predictions={}
    qids=range(total_items)
    ans_file=open('./anns.jsonl','w')
    for i in range(total_items//max_batch_size+1):
        ...
        for qid in batch_qids:
            ...
            idx = problems[qid]["question_id"]
            idxs.append(idx)
            image_file = problems[qid]["image"]
            qs=problems[qid]["text"]
            prompt = ('Instruction: ' +  problems[qid]["text"] + '\n' + 'Response: ').replace("  ", " ").strip()
            ori_prompts.append(qs)
            if problems[qid]['image'] is not None:
                image = Image.open(os.path.join('../data/images/val2014',   'COCO_val2014_'+image_file)).convert('RGB')
                ...
                indicator = 0
            # predictions[qid]={}
            # predictions[qid]['prompt'] = prompt
            # predictions[qid]['image_path'] = os.path.join(image_path, qid, 'image.png') if indicator else None
        ...
        results = generator.generate(
            prompts,images=images,indicators=indicators, max_gen_len=768, temperature=generation_temperature, top_p=top_p,n_feats=n_prompt
        )
        preds.extend(results)


    # print(len(idxs),len(ori_prompts),len(preds))
    for j,output in enumerate(preds):
        ans_id = shortuuid.uuid()
        output = output.strip()
        for pattern in ['Response:']:
            if output.find(pattern) != -1:
                output = output[(output.find(pattern) + len(pattern)):].strip()
        ans_file.write(json.dumps({"question_id": idxs[j],
                                   "prompt": ori_prompts[j],
                                   "text": output.strip(),
                                   "answer_id": ans_id,
                                   "model_id": 'LaVIN',
                                   "metadata": {}}) + "\n")
