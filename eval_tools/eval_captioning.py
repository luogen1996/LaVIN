...
def main(
    ...
    anns=json.load( open('./captions_test5k.json'))['images']
    ...
    for i in range(total_items//max_batch_size+1):
        ...
        for qid in batch_qids:
            ...
            prompt='Instruction: Summarize the visual content of the image.\n' + 'Response: The image features'
            prompt=prompt.replace("  ", " ").strip()


            image = Image.open(os.path.join(image_path, anns[qid]['file_name'])).convert('RGB')
        ...
        for j,result in enumerate(results):
            pred = result.split('The image features')[1]#pattern.findall(result)
            # print(j)
            # print(pred)
    ...
    with open('./cap_preds.json', 'w') as f:
